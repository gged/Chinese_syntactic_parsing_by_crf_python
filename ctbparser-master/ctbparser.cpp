...
bool ctbparser::load_config(char *fn){
	...
	if(!_ns->load("/home/hdz/Desktop/QA/ctbparser-master/GBK2GB2312")){//full path
		cout<<"initialization failed1"<<endl;
		return false;
	}
	...
	if(!_seg->load_model(_seg_model_file)){
		cout<<"initialization failed2"<<endl;
		return false;
	}
	if(!_pos->load_model(_pos_model_file)){
		cout<<"initialization failed3"<<endl;
		return false;
	}
	...
	//}
	if(!check_dict()){
		cout<<"initialization failed4"<<endl;
		return false;
	}
	if(!_dict->Load("__dict")){
		cout<<"initialization failed5"<<endl;
		return false;
	}
	if(!access("__dict",0))
		unlink("__dict");
...
void ctbparser::process_sub2(){
    ...
    char *blank=" ";
	for(i=0;i<1;i++){//tag.size();i++){
		int last_word_end=-1;
		for(j=0;j<=_halfs.size();j++){
			if(j==_halfs.size() || _halfs[j][0]==' '){
			    w.left=last_word_end+1;
				w.right=j-1;
				//cout<<w.left<<" "<<w.right<<endl;
				_words[i].push_back(w);
				last_word_end=w.right+1;
			}
		}
	}
...
void ctbparser::decode_file2(char *fn_in, char *fn_out){
