...
#model_name=modelpath+'CTB_piece_parse_model_1_15'#####2 for test
model_name=modelpath+'CTB_piece_parse_model_0525_no_xing'#####2 for test
#model_name=modelpath+'CTB_piece_parse_model_no_xing_7_5_pieces_len_5_BME'
...
def get_res_kbest(fn,txt=''):#deal only one sen
    ...
    for tag in tags[:]:
        ...
        if tag.find('\t')==-1:#sen probably
            ...
            if len(res)!=0:
                ...
                print 'res=0?piece_parse_model.py#55'
            continue
        tg=tag.split('\t')
        tg=tg[-7:]#maxtag/p,t1/p1,...,t5/p5
        p_node=[]
    ...
    #print len(resl)
    return resl
def piece_parse_model_test(testf):
    #print 'test:',testf
    temp_file=testf+'.tag'
...
def get_res_pack(fn,txt=''):#deal only one sen
    ...
    return res_pack
def piece_parse_model_test_pack(testf):
    #print 'test:',testf
    temp_file=testf+'.tag'
    cmd2='%s -v2 -m %s %s' % (crftest,model_name,testf)
    #cmd2='%s -v2 -m %s %s > %s' % (crftest,model_name,testf,temp_file)
    ...
    return res
def piece_parse_main():
    res=piece_parse_model_test(test_file)
